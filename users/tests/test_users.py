#  Copyright (c) Code Written and Tested by Ahmed Emad in 23/03/2020, 13:58.
#
...
#         """Test updating the user profile for authenticated user with put"""
#         payload = {'name': 'new name', 'password': 'newpassword123'}
#
#         res = self.client.put(ME_URL, payload)
#
#         self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)  # no email in request
#
#         payload2 = {'email': 'test2@gmail.com', 'name': 'new name', 'password': 'newpassword123'}
#         res = self.client.put(ME_URL, payload2)
#
#         self.user.refresh_from_db()
#         self.assertEqual(self.user.name, payload['name'])
#         self.assertTrue(self.user.check_password(payload['password']))
